...
export default function StatsDashboard({ stats, elapsed_stats, edge_stats }) {
  const [expandedNodes, setExpandedNodes] = useState({});
  const [activeTab, setActiveTab] = useState("nodes"); // 'nodes', 'elapsed', 'edges'
  const togglePosthoc = (node) => setExpandedNodes(prev => ({ ...prev, [node]: !prev[node] }));

  if (!stats && !elapsed_stats && !edge_stats) return null;
  ...
  return (
    <div style={{ marginTop: 30, padding: 22, borderRadius: 16, background: "rgba(255,255,255,0.18)", backdropFilter: "blur(10px)", boxShadow: "0 12px 30px rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "right", marginBottom: 20 }}>
        {stats && <button onClick={() => setActiveTab("nodes")} style={{
          marginRight: 10,
          padding: "8px 16px",
          cursor: "pointer",
        ...
        {elapsed_stats && <button onClick={() => setActiveTab("elapsed")} style={{
          marginRight: 10,
          padding: "8px 16px",
          cursor: "pointer",
          fontWeight: activeTab === "elapsed" ? 700 : 400,
          background: activeTab === "elapsed" ? "#eee" : "#fff",
          borderRadius: 8,
          border: "1px solid #ccc"
        }}>Elapsed Time</button>}
        {edge_stats && <button onClick={() => setActiveTab("edges")} style={{
          padding: "8px 16px",
          cursor: "pointer",
      ...
      {/* Node Table */}
      {(activeTab === "nodes" && stats) && <NodeTable stats={stats} expandedNodes={expandedNodes} togglePosthoc={togglePosthoc} />}
      {(activeTab === "elapsed" && elapsed_stats) && <NodeTable stats={elapsed_stats} expandedNodes={expandedNodes} togglePosthoc={togglePosthoc} />}
      {(activeTab === "edges" && edge_stats) && <EdgeTable edge_stats={edge_stats} />}

    </div>
  );
}

function NodeTable({ stats, expandedNodes, togglePosthoc }) {
  return (
